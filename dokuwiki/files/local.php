...
$conf['savedir']     = "{% pillar.get('dokuwiki','./data') %}";          //where to store all the files
$conf['basedir']     = "{% pillar.get('dokuwiki','') %}";                //absolute dir from serveroot - blank for autodetection
$conf['baseurl']     = "{% pillar.get('dokuwiki','') %}";                //URL to server including protocol - blank for autodetect
$conf['cookiedir']   = "{% pillar.get('dokuwiki','') %}";                //path to use in cookies - blank for basedir
$conf['dmode']       = "{% pillar.get('dokuwiki',0755) %}";              //set directory creation mode
$conf['fmode']       = "{% pillar.get('dokuwiki',0644) %}";              //set file creation mode
$conf['allowdebug']  = "{% pillar.get('dokuwiki',0) %}";                 //allow debug output, enable if needed 0|1
/* Display Settings */
$conf['recent']      = "{% pillar.get('dokuwiki',20) %}";                //how many entries to show in recent
$conf['recent_days'] = "{% pillar.get('dokuwiki',7) %}";                 //How many days of recent changes to keep. (days)
$conf['breadcrumbs'] = "{% pillar.get('dokuwiki',10) %}";                //how many recent visited pages to show
$conf['youarehere']  = "{% pillar.get('dokuwiki',0) %}";                 //show "You are here" navigation? 0|1
$conf['fullpath']    = "{% pillar.get('dokuwiki',0) %}";                 //show full path of the document or relative to datadir only? 0|1
$conf['typography']  = "{% pillar.get('dokuwiki',1) %}";                 //smartquote conversion 0=off, 1=doublequotes, 2=all quotes
...
$conf['maxseclevel'] = "{% pillar.get('dokuwiki',3) %}";                 //Up to which level create editable sections (max. 5)
$conf['camelcase']   = "{% pillar.get('dokuwiki',0) %}";                 //Use CamelCase for linking? (I don't like it) 0|1
$conf['deaccent']    = "{% pillar.get('dokuwiki',1) %}";                 //deaccented chars in pagenames (1) or romanize (2) or keep (0)?
$conf['useheading']  = "{% pillar.get('dokuwiki',0) %}";                 //use the first heading in a page as its name
...
$conf['target']['interwiki'] = "{% pillar.get('dokuwiki','') %}";
$conf['target']['extern']    = "{% pillar.get('dokuwiki','') %}";
$conf['target']['media']     = "{% pillar.get('dokuwiki','') %}";
$conf['target']['windows']   = "{% pillar.get('dokuwiki','') %}";
/* Media Settings */
$conf['mediarevisions'] = "{% pillar.get('dokuwiki',1) %}";             //enable/disable media revisions
$conf['refcheck']    = "{% pillar.get('dokuwiki',1) %}";                //check for references before deleting media files
$conf['gdlib']       = "{% pillar.get('dokuwiki',2) %}";                //the GDlib version (0, 1 or 2) 2 tries to autodetect
$conf['im_convert']  = "{% pillar.get('dokuwiki','') %}";               //path to ImageMagicks convert (will be used instead of GD)
$conf['jpg_quality'] = "{% pillar.get('dokuwiki','70') %}";             //quality of compression when scaling jpg images (0-100)
...
$conf['mailprefix']  = "{% pillar.get('dokuwiki','') %}";               //use this as prefix of outgoing mails
$conf['htmlmail']    = "{% pillar.get('dokuwiki',1) %}";                //send HTML multipart mails
...
$conf['send404']     = "{% pillar.get('dokuwiki',0) %}";                //Send a HTTP 404 status for non existing pages?
$conf['broken_iua']  = "{% pillar.get('dokuwiki',0) %}";                //Platform with broken ignore_user_abort (IIS+CGI) 0|1
$conf['xsendfile']   = "{% pillar.get('dokuwiki',0) %}";                //Use X-Sendfile (1 = lighttpd, 2 = standard)
$conf['renderer_xhtml'] = "{% pillar.get('dokuwiki','xhtml') %}";       //renderer to use for main page generation
$conf['readdircache'] = "{% pillar.get('dokuwiki',0) %}";               //time cache in second for the readdir operation, 0 to deactivate.
/* Network Settings */
$conf['dnslookups'] = "{% pillar.get('dokuwiki',1) %}";                 //disable to disallow IP to hostname lookups
// Proxy setup - if your Server needs a proxy to access the web set these
$conf['proxy']['host']    = "{% pillar.get('dokuwiki','') %}";
$conf['proxy']['port']    = "{% pillar.get('dokuwiki','') %}";
$conf['proxy']['user']    = "{% pillar.get('dokuwiki','') %}";
$conf['proxy']['pass']    = "{% pillar.get('dokuwiki','') %}";
$conf['proxy']['ssl']     = "{% pillar.get('dokuwiki',0) %}";
$conf['proxy']['except']  = "{% pillar.get('dokuwiki','') %}";
// Safemode Hack - read http://www.dokuwiki.org/config:safemodehack !
$conf['safemodehack'] = "{% pillar.get('dokuwiki',0) %}";
$conf['ftp']['host'] = "{% pillar.get('dokuwiki','localhost') %}";
$conf['ftp']['port'] = "{% pillar.get('dokuwiki','21') %}";
$conf['ftp']['user'] = "{% pillar.get('dokuwiki','user') %}";
$conf['ftp']['pass'] = "{% pillar.get('dokuwiki','password') %}";
$conf['ftp']['root'] = "{% pillar.get('dokuwiki','/home/user/htdocs') %}";
